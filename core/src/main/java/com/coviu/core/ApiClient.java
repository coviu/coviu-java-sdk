...
import com.coviu.core.auth.Authentication;
import com.coviu.core.auth.OAuth;
...
public class ApiClient {
    ...
    private String basePath = "https://api.coviu.com/v1";
     ...
     */
    public void setCredentials(final String apiKey, final String apiKeySecret, String token) {
        for (Authentication auth : authentications.values()) {
            if (auth instanceof OAuth) {
                OAuth oauth = (OAuth) auth;
                com.coviu.auth.Authorization authz = null;
                if (token != null) {
                    authz = com.coviu.auth.Authorization.load(token);
                }
                oauth.setAuthClient(new AuthClient(new com.coviu.Config() {

                        com.coviu.auth.ClientCredentials creds = new com.coviu.auth.ClientCredentials(apiKey, apiKeySecret);
                        com.coviu.HttpClient.Authenticator basicAuth = new com.coviu.HttpClient.BasicAuthAuthenticator(creds); 
                        com.coviu.HttpClient client = new com.coviu.HttpClient(httpClient);

                        public String getBasePath() {
                            return basePath;
                        }

                        public com.coviu.HttpClient getHttpClient() {
                            return client;
                        }

                        public com.coviu.HttpClient.Authenticator getBasicAuthAuthenticator() {
                            return basicAuth;
                        }
                }, null, authz));
